...
provides useful tools to carlibrate new \cawaqs~model. These features
are not all operative yet. Indeed the majority of them were not taken tken into account for the refactoring of T1 2026. Anyway they are already designed in the code. 
A minimal restucturing and wiring would be neededto bring them operative back. \alertwarning{All the not yet operative functionalities are labelled in the userguide with the acronym WIP (Work in Progress) or with a /!\textbackslash{} symbol in QGIS.}

\begin{itemize}
    \item  \gui{Modify a parametrisation file} (\cref{sec-feat-modify-param}) - WIP: tool for modifying the parameterisation files of the various
        modelled compartments;

    \item  \gui{Statistical Criteria} (\cref{sec-critstat}): a tool for calculating model calibration performance criteria;

    \item \gui{Run-off coefficient calculation}(\cref{sec-runoff-coef}) WIP : a tool for calculating simulated run-off coefficients;

    \item   \gui{Compare sim/obs}(\cref{sec-com-simobs}) WIP: a tool for visualizing simulated and observed time series.
\end{itemize}
...
\subsection{WIP: Modifying Parameterisation Files}
...
\subsection{WIP: Linking an Attribute Table to its Resolution}
...
\subsection{WIP \script{Run-off coefficient calculation}: Calculation of the Run-off Coefficient at Flow Measurement Stations}
...
The \gui{Extract Data from a Sub Area} dialog box (\cref{fig-dialogMask}) allows extracting simulation results over a defined zone. This zone can be specified either as a polygon from a \texttt{QgsVectorLayer} loaded in the project, or as a shapefile imported from a directory.
...
\begin{figure}[H]
    ...
    \caption{\gui{Extract Data from a Sub Area} dialog box.}
    \label{fig-dialogMask}
\end{figure}

Two families of variables can be extracted, each driven by its own set of tickboxes and each producing its own output bundle: the \emph{internal values} (\cref{sec-extract-internal}), i.e.\ the simulated variables of the \script{WATBAL}, \script{HYD} and \script{AQ} compartments inside the polygon, and the \emph{boundary values} (\cref{sec-extract-boundary}), i.e.\ the fluxes crossing the polygon boundary. Both families belong from an independent \gui{GeoPackage} tickbox that switches all the output format.
...
\begin{itemize}
    ...
    \item Extraction polygon: either a shapefile or a \texttt{QgsVectorLayer} present in the project;
    \item Parameters to extract (tickboxes);
    \item The unit of measure, chosen from the combo box next to each parameter (see \cref{sec-extract-units});
    \item Whether the output format should be a GeoPackage or the default loose-file layout (\cref{sec-extract-outputs}).
\end{itemize}

\subsection{Outputs}
\label{sec-extract-outputs}
For each extraction, result files and \texttt{QgsVectorLayer}s are produced. Files are saved in the output directory defined in the main analysis parameters, under
\texttt{PostProcessing/DATA\_EXTRACTED/<Extraction\_Name>}, which contains the following sub-folders:
\begin{itemize}
    \item \textbf{CONFIG/}: configuration files for rerunning simulations WIP;
    \item \textbf{OUTPUTS/}: CSV or GeoPackage files containing the results;
    \item \textbf{TEMP/}: NumPy files used to rerun analyses.
\end{itemize}

The \gui{GeoPackage} tickbox does not \emph{add} a file, it \emph{replaces} the loose files. The two modes are mutually exclusive:
\begin{itemize}
    \item \textbf{Unticked (default)}: one CSV per checked parameter in \texttt{OUTPUTS/} and one \script{.npy} binary per parameter in \texttt{TEMP/} (plus a polygon-total CSV per parameter when weighting is enabled). No GeoPackage is written.
    \item \textbf{Ticked}: a single GeoPackage in \texttt{OUTPUTS/} and nothing else --- no CSV, no \script{.npy}.
\end{itemize}

\subsection{Units}
\label{sec-extract-units}
Each parameter carries its own unit, picked from the combo box next to its checkbox. The \script{WATBAL} fluxes, the \script{AQ} \gui{Recharge} and the \script{HYD} \gui{Flow} are volumetric (\texttt{m\textsuperscript{3}/s} or \texttt{m\textsuperscript{3}/d}); the \script{HYD} \gui{Water Height} is a length (\texttt{m} or \texttt{cm}) and therefore skips the per-day volumetric conversion. The \script{AQ} subsurface (boundary) fluxes have a separate unit combo of their own, described in \cref{sec-extract-boundary}.

\subsection{Parameters --- Internal Values}
\label{sec-extract-internal}
The following variables can be extracted for the surface (\script{WATBAL} compartment of \cawaqs):
\begin{itemize}
    \item \gui{Rainfall};
...
\end{itemize}
For the river system (\script{HYD}):
\begin{itemize}
    \item \gui{Flow};
    \item \gui{Water Height};
\end{itemize}
For the aquifer (\script{AQ}):
\begin{itemize}
    \item \gui{Recharge}.
\end{itemize}

\subsubsection{The internal-values GeoPackage}
\label{sec-extract-internal-gpkg}
When the \gui{GeoPackage} box is ticked, all the checked compartments are bundled into one single file, \texttt{<area>\_InternalValues\_<years>.gpkg}, written in \texttt{OUTPUTS/}. It contains:
\begin{itemize}
    \item one \texttt{cells\_<compartment>} vector layer per mesh (e.g.\ \texttt{cells\_WATBAL}, \texttt{cells\_AQ}, \texttt{cells\_HYD}) holding the masked geometry --- \script{HYD} reaches are clipped to the polygon boundary;
    \item a \texttt{daily\_values} table with one row per (compartment, cell, date, parameter) tuple; it is joined to a mesh layer on the pair (\texttt{compartment}, \texttt{cell\_id});
    \item a \texttt{provenance} table with one row per compartment;
    \item a \texttt{polygon\_total\_<compartment>\_<param>} table per parameter, when weighting is enabled.
\end{itemize}

\subsection{Parameters --- Boundary Values}
\label{sec-extract-boundary}
\begin{itemize}
    \item \gui{Aquifer} subsurface (horizontal) flows: the fluxes crossing the vertical faces of the aquifer cells that lie on the polygon boundary;
    \item \gui{Hydraulic network} WIP: flow of the reaches crossing the boundaries of the mask.
\end{itemize}

\subsubsection{Unit and sign convention}
The \gui{Subsurface flows} checkbox has its own unit combo box, offering two options that differ in \emph{nature}, not merely in scale:
\begin{itemize}
    \item \texttt{m\textsuperscript{3}/d} (default): a daily flow \textbf{rate} (the raw \texttt{m\textsuperscript{3}/s} multiplied by 86\,400), giving one value per day;
    \item \texttt{m\textsuperscript{3}/month}: the actual total \textbf{volume} (\texttt{m\textsuperscript{3}}) that crossed each boundary during each calendar month. The time axis is re-binned to one row per month, each value being the sum, over that month's days, of the daily $\texttt{m\textsuperscript{3}/s} \times 86\,400$. Real month lengths (28, 29, 30 or 31 days) are honoured.
\end{itemize}

\alertinfo{In \texttt{m\textsuperscript{3}/month} mode, a partial first or last month totals \textbf{only its simulated days}. A first or last month whose volume looks small compared to its neighbours is therefore correct, not an error.}

The sign convention travels with the data --- in the CSV header line and in the GeoPackage \texttt{provenance} table: \textbf{positive means a flux entering the cell}.

\subsubsection{Default mode: the face-flux CSV}
With the \gui{GeoPackage} box unticked, the boundary fluxes are written to a loose face-flux CSV. Faces are named according to the convention \texttt{nCell(ID\_ABS)\_direction\_unit}, for example \texttt{3323\_west\_m3d}. This is the \textbf{only} mode that exposes the per-face \emph{gross} detail, i.e.\ each face's own value taken separately.

\subsubsection{GeoPackage mode: the AQ-boundary GeoPackage}
\label{sec-extract-boundary-gpkg}
The AQ-boundary GeoPackage is exclusive in the same way as the internal-values one: when the box is ticked, a single \texttt{<area>\_AqBoundary\_<years>.gpkg} is written \emph{instead of} the loose face-flux CSV. It carries:
\begin{itemize}
    \item one \texttt{cells\_AQ\_layer<id>} geometry layer per aquifer layer;
    \item a values table --- \texttt{daily\_values} in \texttt{m\textsuperscript{3}/d} mode, \texttt{monthly\_values} in \texttt{m\textsuperscript{3}/month} total-volume mode.
\end{itemize}
...
Both the geometry layers and the values table carry a \texttt{faces} column: the comma-separated list of the cardinal directions each boundary cell touches, for example \texttt{'north,west'}. Each row remains \textbf{one net series per cell}; the \texttt{faces} column simply says which directions contributed to it. The \texttt{faces} column also appears in the attribute table of the QGIS borders layer registered in the project, regardless of the GeoPackage mode.

\paragraph{Per-face structure columns.} Inside the GeoPackage file only, seven additional columns spread that summary across queryable fields. They appear on both the \texttt{cells\_AQ\_layer<id>} geometry layers and the values table, but \textbf{not} on the registered QGIS borders layer, which keeps only \texttt{faces}.
\begin{itemize}
    \item \texttt{n\_faces}: how many cardinal sides the cell borders (1 to 3);
    \item \texttt{face1\_orient}/\texttt{face1\_outid} \dots{} \texttt{face3\_orient}/\texttt{face3\_outid}: two columns per face slot.
\end{itemize}
\texttt{faceN\_orient} is the $N$-th bordered direction; slots beyond \texttt{n\_faces} are left blank. \texttt{faceN\_outid} is filled \textbf{only} when that face's flux was sourced from the outside cell --- the \emph{coarse-inside} case, where the reported value came from the smaller outside neighbours, whose identifiers are then comma-joined into the field. It stays blank for a normal own-face read.
