...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\linewidth]{
        3_features/fig/DialogExtract.png
    }
    \caption{Boite de dialogue \gui{Extract Data from a Sub Area} - version sans la possibilite de choisir le period}
    \label{fig-dialogHydroRegime}
\end{figure}
...
\subsection*{Typologie des sorties par compartiment}
Les sorties dépendent des cases cochées dans la colonne de droite.

\subsubsection*{Sorties -- Bilan hydrologique (WATBAL)}
...
\subsubsection*{Sorties -- Régime hydrologique (HYD)}
\gui{Surface flow}
...
\subsubsection*{Sorties -- Aquifère (AQ)}
\gui{Subsurface flow}
