...
Here reported the information from the \href{https://pixi.prefix.dev/latest/installation/#__tabbed_1_1}{Pixi Installation page} :
...
# 2. Build the ZIP from the parent directory, excluding (optionally) dev-only artefacts 
cd ..
...
\section{Advanced Installation Settings (DEV)} \label{setup-dependances}
...
The core Python dependencies are: \script{pandas}, \script{numpy}, \script{ipython}, \script{matplotlib}, and \script{geopandas}. As a manual fallback, open a QGIS Python console\footnote{By clicking on \includegraphics[height=0.45cm]{1_installation_requirements/fig/interpreteur_python.png} in the QGIS interface.} and run:
...
\subsection{Developer Installation} \label{sec-install-expert}
...
The plug-in code structure is described in section \ref{chap-description-src}.\\
